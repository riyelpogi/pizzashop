<div class="w-full relatuive flex flex-col justify-center items-center">
    @foreach ($orders as $order)
    @foreach ($orders as $order)
    <div class="w-9/12 m-5 rounded-lg relative p-2 " style="background-color: #F48B29;">
        ...
        <div class="w-full relative mt-2">
            ...
            <h1 class="text-xs font-semibold">Date Delivered: {{strtoupper($order->date_delivered)}}</h1>
        </div>
        ...
        <div class="w-full flex justify-end mt-2">
            <div class="flex gap-2 justify-center items-center">
                ...
                @if ($order->status == 'pending')
                     <x-button wire:click="preparing({{$order->id}})"  wire:key="status-({{$order->id}})">Mark as preparing</x-button>

                @elseif ($order->status == 'preparing')
                     <x-button wire:click="delivering({{$order->id}})"  wire:key="status-({{$order->id}})">mark as  delivering</x-button>

                @elseif ($order->status == 'delivering')
                <x-button wire:click="delivered({{$order->id}})"  wire:key="status-({{$order->id}})">mark as delivered</x-button>
